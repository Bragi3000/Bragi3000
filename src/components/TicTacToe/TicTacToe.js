...
export function checkWin(squares, size) {
    ...
    for (let i = 0; i < size; i++) {
        const row = matrix[i];
        if (row.every(e => e === row[0])) { return row[0];}
    }
    ...
    for (let i = 0; i < size; i++) {
        ...
        if (column.every(e => e === column[0])) { return column[0];}
    }
    ...
    if (first_diagonal.every(e => e === first_diagonal[0])) { return first_diagonal[0];}
    if (second_diagonal.every(e => e === second_diagonal[0])) { return second_diagonal[0];}
